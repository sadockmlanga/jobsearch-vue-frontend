<template>
  <Breadcrumb breadcrumb="Applications" />
...
onMounted(async () => {
  try {
    await dash.fetchAllApplications();
    
    tableData.value = dash.allApplications.map((item: any, index: number) => ({
      id: index + 1,
      dataId: item.id,
...
const filteredData = computed(() => {
  ...
  return tableData.value.filter(item =>
    item.fullName.toLowerCase().includes(query) || item.job.toLowerCase().includes(query)
  );
});
