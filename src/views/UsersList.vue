<template>
  <Breadcrumb breadcrumb="UsersList" />
...
onMounted(async () => {
  try {
    await dash.fetchUsers();
    tableData.value = dash.users.data.map((item: any, index: number) => ({
      id: index + 1,
...
const columns = ref(['ID', 'Firs Name', 'LastName', 'Action']);
...
const filteredData = computed(() => {
  ...
  return tableData.value.filter(item =>
    item.fullName.toLowerCase().includes(query) || item.email.toLowerCase().includes(query)
  );
});
